...
// ----------------------------------------------------
class LocalDB {
  private getTable(key: string, defaultVal: any) {
  ...
  async getTeamMembers(projectId?: string) {
    ...
    return projectId ? list.filter((t: any) => t.project_id === projectId) : list
  }
  async saveTeamMember(member: any) {
    const list = this.getTable('team_members', defaultTeamMembers)
    if (member.id) {
      const idx = list.findIndex((m: any) => m.id === member.id)
  ...
  async deleteTeamMember(id: string) {
    const list = this.getTable('team_members', defaultTeamMembers)
    this.setTable('team_members', list.filter((m: any) => m.id !== id))
  }
  ...
  async deleteTask(id: string) {
    const list = this.getTable('tasks', defaultTasks)
    this.setTable('tasks', list.filter((t: any) => t.id !== id))
  }

  // Milestones
  async getMilestones(projectId?: string) {
    const list = this.getTable('milestones', defaultMilestones)
    return projectId ? list.filter((m: any) => m.project_id === projectId) : list
  }
  async saveMilestone(milestone: any) {
    const list = this.getTable('milestones', defaultMilestones)
    if (milestone.id) {
      const idx = list.findIndex((m: any) => m.id === milestone.id)
  ...
  async deleteMilestone(id: string) {
    const list = this.getTable('milestones', defaultMilestones)
    this.setTable('milestones', list.filter((m: any) => m.id !== id))
  }

  // Channels
  async getChannels(projectId?: string) {
    const list = this.getTable('channels', defaultChannels)
    return projectId ? list.filter((c: any) => c.project_id === projectId) : list
  }
  async saveChannel(channel: any) {
    const list = this.getTable('channels', defaultChannels)
    if (channel.id) {
      const idx = list.findIndex((c: any) => c.id === channel.id)
  ...
  async deleteChannel(id: string) {
    const list = this.getTable('channels', defaultChannels)
    this.setTable('channels', list.filter((c: any) => c.id !== id))
  }

  // Stakeholders
  async getStakeholders(projectId?: string) {
    const list = this.getTable('stakeholders', defaultStakeholders)
    return projectId ? list.filter((s: any) => s.project_id === projectId) : list
  }
  async saveStakeholder(stakeholder: any) {
    const list = this.getTable('stakeholders', defaultStakeholders)
    if (stakeholder.id) {
      const idx = list.findIndex((s: any) => s.id === stakeholder.id)
  ...
  async deleteStakeholder(id: string) {
    const list = this.getTable('stakeholders', defaultStakeholders)
    this.setTable('stakeholders', list.filter((s: any) => s.id !== id))
  }

  // Escalation Levels
  async getEscalations(projectId?: string) {
    const list = this.getTable('escalations', defaultEscalations)
    return projectId ? list.filter((e: any) => e.project_id === projectId) : list
  }
  async saveEscalation(escalation: any) {
    const list = this.getTable('escalations', defaultEscalations)
    if (escalation.id) {
      const idx = list.findIndex((e: any) => e.id === escalation.id)
  ...
  async deleteEscalation(id: string) {
    const list = this.getTable('escalations', defaultEscalations)
    this.setTable('escalations', list.filter((e: any) => e.id !== id))
  }
  ...
  async deleteMeeting(id: string) {
    const list = this.getTable('meetings', defaultMeetings)
    this.setTable('meetings', list.filter((m: any) => m.id !== id))
  }

  // Integrations (Credentials)
  async getIntegrations(projectId?: string) {
    const list = this.getTable('integrations', defaultIntegrations)
    return projectId ? list.filter((i: any) => i.project_id === projectId) : list
  }
  async saveIntegration(integration: any) {
    const list = this.getTable('integrations', defaultIntegrations)
    if (integration.id) {
      const idx = list.findIndex((i: any) => i.id === integration.id)
  ...
  async deleteIntegration(id: string) {
    const list = this.getTable('integrations', defaultIntegrations)
    this.setTable('integrations', list.filter((i: any) => i.id !== id))
  }
}
...
export const db = {
  ...
  projects: {
    ...
    save: async (proj: any): Promise<any> => {
      if (supabase) {
        const payload = {
  ...
  team_members: {
    ...
    save: async (member: any): Promise<any> => {
      if (supabase) {
        const payload = {
  ...
  tasks: {
    ...
    save: async (task: any): Promise<any> => {
      if (supabase) {
        const payload = {
  ...
  channels: {
    list: async (projectId?: string): Promise<any[]> => {
      if (supabase) {
        ...
        const { data, error } = await q
        if (error) throw error
        return (data || []).map((d: any) => ({ ...d, type: d.channel_type }))
      }
      return localDB.getChannels(projectId)
    },
    save: async (channel: any): Promise<any> => {
      if (supabase) {
        const payload = {
  ...
  escalations: {
    ...
    save: async (esc: any): Promise<any> => {
      if (supabase) {
        const payload = {
  ...
  integrations: {
    ...
    save: async (integration: any): Promise<any> => {
      if (supabase) {
        const payload = {
